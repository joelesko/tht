...
class u_Web extends StdModule {
    ...
    function u_request_headers () {
        return Owl::getWebRequestHeaders();
    }

    function u_request_header ($val) {
        return Owl::getWebRequestHeader($val);
    }
    ...
    function u_request () {
        ...
        if (!$this->request) {
            ...
            $ip = Owl::getPhpGlobal('server', 'REMOTE_ADDR');
            ...
            $r = [
                ...
                'userAgent'   => Owl::getPhpGlobal('server', 'HTTP_USER_AGENT'),
                'method'      => strtolower(Owl::getPhpGlobal('server', 'REQUEST_METHOD')),
                'referrer'    => Owl::getPhpGlobal('server', 'HTTP_REFERER', ''),
            ...
            $hostWithPort = Owl::getPhpGlobal('server', 'HTTP_HOST');
    ...
    function isHttps () {
        $https = Owl::getPhpGlobal('server', 'HTTPS', '');
        $port = Owl::getPhpGlobal('server', 'SERVER_PORT');

		return (!empty($https) && $https !== 'off') || intval($port) === 443;
    }

    function isAjax () {
        $requestedWith = Owl::getWebRequestHeader('x-requested-with');
        return (strtolower($requestedWith) === 'xmlhttprequest');
    }

    function relativeUrl() {
        $path = Owl::getPhpGlobal('server', "REQUEST_URI");  // SCRIPT_URL
        if (!$path) {
            // Look for PHP dev server path instead.
            $path = Owl::getPhpGlobal('server', "SCRIPT_NAME");
            if (!$path) {
                Owl::configError("Unable to determine route path.  Only Apache and PHP dev server are supported.");
            }
        }
        return $path;
    }
    ...
    function languages () {
        $langs = [];
        $acceptLang = strtolower(Owl::getPhpGlobal('server', 'HTTP_ACCEPT_LANGUAGE', ''));
        if ($acceptLang) {
    ...
    function u_send_js ($chunks) {
        ...
        print "(function(){\n";
        foreach ($chunks->val as $c) {
            print Owl::module('Js')->u_minify(OLockString::getUnlocked($c));
        }
        print "\n})();";
        $this->endGzip();
    }
    ...
    function u_send_page ($doc) {
       ...
       // $jsData = Owl::module('Js')->serializeData();
...
HTML;
    ...
    function u_send_error ($code, $title='') {
        ...
        if ($code !== 500) {
            ...
            Owl::runStaticRoute($code);
        }

        // User custom error page
        // $errorPage = Owl::module('File')->u_document_path($code . '.html');
    ...
    function assetTags ($type, $paths, $incTag, $blockTag) {
        ...
        foreach ($paths as $path) {
            if (OLockString::isa($path)) {
                ...
                $tag = str_replace('{NONCE}', Owl::data('cspNonce'), $tag);
                $blocks []= $tag;
            }
            else {
                // Link to asset, with cache time set to file modtime
                $basePath = preg_replace('/\?.*/', '', $path);
                $filePath = Owl::getOwlFileName(Owl::path('pages', $basePath));
    ...
    function u_parse_html($raw) {
        return Owl::parseTemplateString('html', $raw);
    }
    ...
    function u_breadcrumbs($links, $joiner = '&gt;') {
        $aLinks = [];
        foreach ($links as $l) {
            $aLinks []= Owl::module('Web')->u_link($l['label'], $l['url'])->u_unlocked();
        }
    ...
    function u_mask_email($email) {
        ...
        // if (!Owl::module('User')->u_logged_in() && $placeholder) {
        //     return $placeholder;
        // }
        ...
        $r = strtolower(Owl::module('String')->u_random(rand(6,12)));
        $r = preg_replace('/[^a-z]/', '', $r);

        $r2 = strtolower(Owl::module('String')->u_random(rand(6,12)));
        $r2 = preg_replace('/[^a-z]/', '', $r2);

        $e = Owl::module('String')->u_random(rand(5,80));
    ...
    function u_temp_get_input($method, $name, $type='token') {
        ...
        $val = trim(Owl::getPhpGlobal($method, $name, false));
    ...
    function u_get_data($vid, $method) {
        ...
        foreach ($ruleset as $name => $sRules) {
            $rawVal = Owl::getPhpGlobal($method, $name, false);
    ...
    function u_data ($method, $key, $type='token') {
        if ($method === 'route') {
            ...
            Owl::error("Unknown method '$method' for key '$key'");
        }
    }
