...
class PhpObject {
    ...
    function __call ($func, $args) {
        ...
        Owl::module('Php')->u_call($this->prefix . $f, $args);
    }
}
...
class u_Php extends StdModule {
    ...
    function checkPhpFunction ($func) {
       ...
       // return;
        ...
        // $allowed = preg_split('/\s*,\s*/', Owl::getConfig('allowedPhpFunctions'));
        // foreach ($allowed as $fn) {
        //     $match = false;
        //     if (substr($fn, strlen($fn)-1, 1) === '*') {
        //         if (strlen($fn) < 3) {
        //             Owl::error("Wildcard '$fn' in 'allowedPhpFunctions' must be 2+ characters long.");
        //         }
        ...
        //     }
        // }
        // Owl::error("PHP function '$func' not listed in 'allowedPhpFunctions' config.", $allowed);
    }
