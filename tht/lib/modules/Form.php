...
class u_Form extends StdModule {
    ...
    function u_open ($actionUrl='', $atts=[], $honeyPot=true) {
        ...
        if (!$actionUrl) { $actionUrl = Tht::getPhpGlobal('server', 'SCRIPT_NAME'); }
        ...
        if (isset($atts['fileUpload']) && $atts['fileUpload']) {
            if (!Tht::getConfig('allowFileUploads')) {
                Tht::error("To enable file uploads, you must also set `allowFileUploads: true` in `settings/app.jcon`");
            }
    ...
    //     $ssl = ssl_
    //
    //     $full = md5(Tht::getPhpGlobal('server', 'CLIENT_IP') . '###' . Tht::getPhpGlobal('server', 'SCRIPT_FILENAME'));
    //     $keyLen = 8;
    //     $key = [
    ...
    // function honeypot() {
    //     $secret = Tht::getPhpGlobal('server');
    //
    //
    // }
    //
