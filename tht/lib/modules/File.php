...
class u_File extends StdModule {
    ...
    function _call ($fn, $args=[], $checkFileExists='', $checkReturn=true) {
        ...
        if ($checkFileExists) {
            ...
            if (!file_exists($checkFileExists)) {
                ...
                Tht::error("File does not exist: `" . Tht::getRelativePath('root', $checkFileExists) . "`");
            }
        }
        ...
        if (!$checkReturn && $ret === FALSE) {
            ...
            Tht::error("File Error: `" . realpath($fileToCheck) . "`");
        }
    ...
    function u_log ($data, $fileName='app.log') {
        ...
        return u_File::_call('file_put_contents', [Tht::path('logs', $fileName), [$line], [LOCK_EX|FILE_APPEND]]);
    }
    ...
    function u_data_path ($relPath) {
        return Tht::getAppDataPath($relPath);
    }

    function u_document_path ($relPath) {
        return Tht::makePath(Tht::getPhpGlobal('server', 'DOCUMENT_ROOT'), $relPath);
    }
