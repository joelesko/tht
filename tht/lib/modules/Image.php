...
class u_Image {
    ...
    function getImageSize($oldFile) {
        ...
        if ($oldSizeX >= $max || $oldSizeY >= $max) {
            ...
            Tht::error("Can't handle dimension larger than $max. Got: `$oldSizeX x $oldSizeY` for `$fileName`");
        }
    ...
    function auditImages($dir) {
        ...
        $images = Tht::module('*File')->u_for_files($dir, function($f) use (&$numImages, &$oldSizeTotal, &$newSizeTotal) {
