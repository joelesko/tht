...
class u_Page_Object extends OClass {
    ...
    private static $didSendPage = false;
        ...
        if ($type) {
            ...
            if (OTypeString::isa($val)) {
                ...
                if ($gotType != $type && $gotType != $part) {
                    OTypeString::assertType($val, $type);
                }
            }
        }
    ...
    function getRenderedParts() {
        ...
        if ($part['body']) {
            $val['body'] = $this->htmlSection('body', $part['body'], 'skipWrap');
        }
        else {
            $val['body'] = '';
    ...
    function htmlSection($part, $html, $skipWrap=false) {
    ...
    function validateAssetUrl($url) {
        ...
        $fullPublicPath = Tht::path('public', $relPath);

        if (!file_exists($fullPublicPath)) {
            $this->error("Asset file does not exist: `$fullPublicPath`");
        }
    }
}