...
class u_Image_Optimizer {
    ...
    public function optimize($oldFileName, $maxResizeWidth=1200, $fileTag='optimized') {
        ...
        $perfTask = Tht::module('Perf')->u_start('Image.optimize', $this->oldFileName);
    ...
    function init() {
        ...
        if (str_contains($this->oldFileName, '_' . $this->fileTag)) {
    ...
    function getNewImageSize() {
        ...
        if (str_contains($this->oldFileName, '@')) {
            return $oldSize;
        }
    ...
    function getImageSize($fileName) {
        ...
        if ($oldSizeX >= $max || $oldSizeY >= $max) {
            ...
            $this->error("Can't handle image dimension larger than $max.  Got: `$oldSizeX x $oldSizeY` for `$fileName`");
        }
