...
class u_Output extends OStdModule {
    ...
    function u_set_header($name, $value, $flags=null) {
        ...
        if ($this->sentResponseType) {
            $this->error("Can't set response header `$name` because output was already sent.");
        }
    ...
    function sendByType($out) {
        ...
        if ($out === true) {
            // Refresh
            // TODO: make this more explicit, like 'return -refresh'
    ...
    function u_send_json($jsonOrMap, $expiryDelta=0) {
        ...
        if (OMap::isa($jsonOrMap)) {
            ...
            $this->argumentError('Argument #1 must be of type: `JsonTypeString` or `Map`', 'sendJson');
        }
    ...
    function u_send_error($code, $title='', $descHtml='') {
        ...
        if (!Tht::module('Request')->u_is_ajax()) {
            ...
            $msg = $this->getErrorTitleAndMessage($code);
            ...
            if ($descHtml) {
                $msg['desc'] = OTypeString::getUntyped($descHtml, 'html');
            }
            ...
            <h1 style="margin-top: 1em;"><?= $msg['title'] ?></h1>
            <div style="margin-top: 1em;"><?= $msg['desc'] ?></div>
            <div style="margin-top: 3em"><a style="text-decoration: none; font-size: 20px;" href="/">Home Page &rarr;</a></div></div>
            </body></html><?php
        }
    ...
    function getErrorTitleAndMessage($code) {

        $defaultMessages = [

            '404' => ['title' => 'Page Not Found', 'desc' => "Sorry, we couldn't find the page you were looking for."],
            '500' => ['title' => 'Website Error',  'desc' => 'Sorry, something went wrong on our end.'],
        ];

        return $defaultMessages[$code] ?? $defaultMessages['500'];
    }

    // TODO: convert to Brotli
    // https://github.com/vdechenaux/brotli-php
    function startGzip($forceGzip=false) {

        if ($this->gzipBufferOpen) { return; }

        if ($forceGzip || Tht::getThtConfig('compressOutput')) {
    ...
    function minifyJs($str) {
        ...
        Tht::loadLib('lib/core/utils/Minifier.php');
...
class AssetOptimizer {
    ...
    function scan($plainText) {
        ...
        $config = Tht::getThtConfig('optimizeAssets');
        ...
        if (in_array('images', $configFlags)) {
            $altHelpMsg = 'Other Solution: // Remove `images` from `optimizeAssets` in `config/app.jcon`. (Ex: optimizeAssets: minify|gzip)';
            Tht::checkRequiredPhpLib('gd', $altHelpMsg);
        }
        ...
        $fnUpdateUrl = function($m) use ($configFlags) {
            ...
            if ($ext == 'js' || $ext == 'css') {
                if (in_array('minify', $configFlags) || in_array('gzip', $configFlags)) {
                    $perfTask = Tht::module('Perf')->u_start('Output.optimizeAsset', $fullPath);
                ...
                if (in_array('images', $configFlags)) {
                    $perfTask = Tht::module('Perf')->u_start('Output.optimizeImage', $fullPath);
                    $newPath = $this->optimizeImage($fullPath);
                    $perfTask->u_stop();
                }
            }

            $newUrl = (new FileTypeString($newPath))->u_to_url();
        ...
        $perfTask = Tht::module('Perf')->u_start('Output.optimizeAssets');
    ...
    function optimizeImage($fullPath) {
        ...
        require_once('Image/ImageOptimizer.php');
    ...
    function minifyAsset($type, $origFullPath) {
        ...
        $minContent = PathTypeString::create($origFullPath)->u_read(OMap::create(['join' => true]));
    ...
    //     $perfTask = Tht::module('Perf')->u_start('Web.assetUrl', $url->u_render_string());
