...
class u_Db extends OStdModule {
    ...
    private function connect() {
        ...
        $perfTask = Tht::module('Perf')->u_start('db.connect', $dbId);
        ...
        if ($dbConfig['driver'] == 'sqlite') {
            ...
            if (!file_exists($dbFilePath)) {
                $this->error("Unable to find database file: `$dbFilePath`");
            }
    ...
    function filterInsertVals($rawVals) {
        ...
        foreach ($rawVals as $k => $val) {
            if (is_object($val)) {
                 $sqlVals[$k] = $val->u_z_to_sql_string();
            }
            else {
                $sqlVals[$k] = $val;
            }
    ...
    function query($sqlTypeString) {
        ...
        $perfTask = Tht::module('Perf')->u_start('Db.query', $sql);
        Tht::module('Meta')->u_fail_if_template_mode();
    ...
    function executeQuery($sql, $params) {
        ...
        $durationSecs = v($durationMs / 1000)->u_floor(2);

        if ($durationSecs >= Tht::getThtConfig('logSlowDbQuerySecs')) {
            Tht::module('Log')->u_warn(OMap::create([
                'message'      => 'Slow Query',
