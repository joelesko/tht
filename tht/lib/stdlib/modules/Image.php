...
class u_Image_Object extends OClass {
    ...
    function u_save($fFilePath, $flags=null) {

        $this->ARGS('*m', func_get_args());

        OTypeString::assertType($fFilePath, 'file');
        $sFilePath = $fFilePath->u_render_string();
        ...
        $foundExt = preg_match('/\.([a-zA-Z]{3,4})/', $sFilePath, $m);
        if (!$foundExt) {
            $this->error("Missing file extension for file: `$sFilePath`");
        }
        $ext = $m[1];
        if ($ext == 'jpg') { $ext = 'jpeg'; }
        if ($ext == 'ico') { $ext = 'bmp'; }
        ...
        if (!in_array($ext, u_Image::$supportedExts)) {
            $this->error("Unsupported extension `$ext` for file: `$sFilePath`");
        }
        ...
        $result = false;
        switch ($ext) {
            case 'bmp':  $result = imagebmp($this->gdi,  $sFilePath,  false);    break;
            case 'jpeg': $result = imagejpeg($this->gdi, $sFilePath,  $flags['quality']);           break;
            case 'webp': $result = imagewebp($this->gdi, $sFilePath,  $flags['quality']);           break;
            case 'png':  $result = imagepng($this->gdi,  $sFilePath,  $flags['pngCompression']);    break;
            case 'gif':  $result = imagegif($this->gdi,  $sFilePath);                               break;
        }
        set_time_limit($prevTimeoutLimit);

        if (!$result) {
            $this->error("Unable to save image: `$sFilePath`");
        }
...
*/
    ...
    function u_draw_text($msg, $flags=null) {
        ...
        if ($flags['font'] == 'default') {
            $flags['fontPath'] = Tht::systemPath('/lib/stdlib/modules/Image/resources/ibm_plex_condensed_min.ttf');
        }
        else {
