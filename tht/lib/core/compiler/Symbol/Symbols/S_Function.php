...
class S_Function extends S_Statement {
    ...
    function asStatement($p) {
        ...
        if ($p->symbol->token[TOKEN_TYPE] === TokenType::WORD) {
            ...
            if ($this->type == SymbolType::NEW_TEMPLATE) {
                preg_match('/(' . CompilerConstants::TEMPLATE_TYPES_RX .')$/i', $sName, $m);
                $sTemplateType = $m[1]; // was already validated in the Tokenizer
            }

            // PHP doesn't allow names in dynamic functions.
            // We need to allow it for anonymous templates with type suffixes.
    ...
    function parseArgs($p, $hasName) {
        ...
        // List of args.  function foo($arg1, $arg2) { ... }
        if ($p->symbol->isValue("(")) {
            ...
            $space = $hasName ? 'x(x' : ' (x';
            $p->now('(', 'function.args.open')->space($space)->next();
            ...
            if (count($argSymbols) > CompilerConstants::MAX_FUN_ARGS) {
                ...
                $p->error('Can\'t have more than ' . CompilerConstants::MAX_FUN_ARGS . ' arguments to a function.  Try: Combine arguments into a Map', $sOpenParen->token);
            }
...
trait ClosureVarParser {

    // Just import everything from outer scope.
    // TODO: only use vars that are actually used inside the inner scope.
