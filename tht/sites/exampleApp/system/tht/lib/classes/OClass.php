...
class OClass implements \JsonSerializable {
    ...
    function addErrorHelpLink($method = '') {
        ...
        // $classToken = strtolower(v($className)->u_slug());
        // $docUrl = '/manual/class/' . $classToken;
        // $label = $this->getErrorClass();
        // if ($contextMethod) {
        //     $methToken = strtolower(v($contextMethod)->u_slug());
        //     $docUrl .= '/' . $methToken;
        //     $label .= '.' . v(unu_($contextMethod))->u_camel_case();
        // }

        // ErrorHandler::setHelpLink($docUrl, $label);
    }
    ...
    function argumentError($msg, $method) {
        ...
        $methodLabel = v($method)->u_camel_case();

        $label = $this->getClass() . '.' . $methodLabel;

        ErrorHandler::setHelpLink('/manual/class/' . strtolower($this->getClass()) . '/' . $methodToken, $label);
        ErrorHandler::addOrigin('stdClass.' . strtolower($this->getClass()));
    ...
    function ARGS($sig, $args) {
        ...
        if ($err) {
            $this->error($err['msg'], $err['function']);
        }
    }
