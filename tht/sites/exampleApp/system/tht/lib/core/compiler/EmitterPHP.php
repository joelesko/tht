...
class EmitterPHP extends Emitter {
    ...
    function pFunction ($value, $k) {
        ...
        $t = "function ### (###) ### {\n %!WRAP% %!IMPLICIT% ### return \o\ONothing::create(__METHOD__); }";
        ...
        foreach ($this->constantValues as $cv) {
            ...
            if ($cv['___type'] == 'map') {
                $objectWrappers .=
                    "$varName = is_object($varName) ? $varName : \o\OMap::create($varName);\n";
            }
            else {
                $objectWrappers .=
                    "$varName = is_object($varName) ? $varName : \o\OList::create($varName);\n";
            }
        }
