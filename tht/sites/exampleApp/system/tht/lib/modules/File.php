...
class u_File extends OStdModule {
    ...
    function u_read ($fileName, $asString=false) {
    ...
    function u_write ($filePath, $data, $mode='replace') {
        ...
        if (!in_array($mode, ['replace', 'append', 'restore'])) {
            $this->error("Unknown write mode `$mode`. Supported modes: `replace` (default), `append`, `restore`");
        }
    ...
    function u_log ($data, $fileName='app.log') {
        ...
        if (is_array($data) || is_object($data)) {
            $data = Tht::module('Json')->u_format($data, true);
        }
        else {
            $data = trim($data);
            $data = str_replace("\n", '\\n', $data);
    ...
    function u_delete ($filePath) {
        ...
        if (is_dir($filePath)) {
            $this->error("Argument 1 for `delete` must not be a directory: `$filePath`.  Suggestion: `File.deleteDir()`");
        }
    ...
    // TODO: merge with u_for_files
    function u_read_dir ($d, $filter = 'none') {

        $this->ARGS('ss', func_get_args());
        if (!in_array($filter, ['none', 'files', 'dirs'])) {
            $this->error("Unknown filter `$filter`. Supported filters: `none` (default), `files`, `dirs`");
        }
        ...
        return OList::create($filteredFiles);
    }

    function u_loop_dir($dirPath, $fn, $goDeep=false) {

        $this->ARGS('scb', func_get_args());
        ...
        while (true) {
            ...
            if ($goDeep && $isDir) {
                ...
                $fileInfo = $this->u_get_parts($subPath);
    ...
    function u_parse_csv($fileName, $assignKeys=false) {
