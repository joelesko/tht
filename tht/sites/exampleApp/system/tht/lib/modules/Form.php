...
require_once('helpers/FormObject.php');
...
class u_Form extends OStdModule {
    ...
    function u_create ($formId, $formConfig) {
        ...
        foreach ($formConfig as $k => $v) {
            if (!OMap::isa($v)) {
                $this->error("Form config field `$k` must contain a Map.");
            }
        }

        $form = new Form ($formId, $formConfig);
