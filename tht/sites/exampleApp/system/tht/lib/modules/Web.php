...
class u_Web extends OStdModule {
    ...
    function u_js_tag($origUrl) {

        $this->ARGS('*', func_get_args());

        $path = OTypeString::getUntyped($origUrl, 'url');

        $newUrl = $this->assetUrl('js', $origUrl);
    ...
    function u_css_tag($origUrl) {

        $this->ARGS('*', func_get_args());

        $path = OTypeString::getUntyped($origUrl, 'url');

        $newUrl = $this->assetUrl('css', $origUrl);
    ...
    // TODO: Hook into image optimization.
    // Add a 'v' param with the asset's modtime.
    function u_asset_url($url) {
        ...
        // Already has a 'v' param.
        if ($url->u_get_query()->u_get('v', 'i')) {
            return $url;
        }
        ...
        $relPath = $url->u_get_path();
        $fullPath = Tht::path('public', $relPath);
    ...
    function assetUrl($assetType, $origUrl) {
        ...
        $fileExt = $origUrl->u_get_file_parts()['fileExt'];
        if (!$fileExt) {
            return $origUrl->u_render_string();
        }
    ...
    // TODO: Clean up config-based logic (move to an init function)
    function minifyAsset($type, $origRelPath) {

        $config = Tht::getConfig('optimizeAssets');

        $origFullPath = Tht::path('public', $origRelPath);
        if (!file_exists($origFullPath)) {
            $this->error("Asset file does not exist: `$origFullPath`", $type . 'Link');
        }
        ...
        $minContent = Tht::module('*File')->u_read($origFullPath, true);
